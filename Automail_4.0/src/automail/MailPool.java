...
public class MailPool {
	...
	private void loadItem(ListIterator<Robot> i) throws ItemTooHeavyException {
		Robot robot = i.next();
		assert(robot.getStorage().isEmpty() == 1);
		// System.out.printf("P: %3d%n", pool.size());
		ListIterator<Item> j = pool.listIterator();
		if (pool.size() > 0) {
			try {
			if (robot.hasHand() == 1) {
				robot.getStorage().addToHand(j.next().mailItem); // hand first as we want higher priority delivered first
				j.remove();
			}
			if (pool.size() > 0 && robot.hasTube() == 1 && robot.getStorage().tubeIsFull() == 0) {
				robot.getStorage().addToTube(j.next().mailItem); 
				j.remove();
			}
